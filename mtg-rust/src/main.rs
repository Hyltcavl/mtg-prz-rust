...
mod test;
mod utils;
use cards::card::{ScryfallCard, VendorCard};
use dotenv;
use env_logger;
use log;
...
async fn prepare_dl_cards() -> Result<String, Box<dyn std::error::Error>> {
    ...
    for card in &cards {
        grouped_cards
            .entry(card.name.cleaned.to_string())
...
async fn main() {
    ...
    let mut dl_cards: HashMap<String, Vec<VendorCard>> = HashMap::new();
    if !dl {
        ...
        .unwrap();
        dl_cards =
            read_json_file::<HashMap<String, Vec<VendorCard>>>(path.to_str().unwrap()).unwrap();
    } else {
        dl_cards =
            read_json_file::<HashMap<String, Vec<VendorCard>>>(&dl_cards_path.unwrap()).unwrap();
    }
